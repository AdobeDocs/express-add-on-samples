...
export class OAuthUtils {
    ...
    async generateAccessToken(tokenRequest) {
        const requestData = {
            ...
            code: decodeURIComponent(tokenRequest.code),
...
async function saveTokenResponse(store, id, clientId, requestData, tokenUrl) {
    const options = {
        method: "POST",
    ...
    if (!response.ok) {
        throw new Error(`Failed to get the access token for request id: ${id}.`, data);
    }
    ...
    const tokenResponse = {
        ...
        refreshToken: data.refresh_token ?? requestData.refresh_token,
